...
def creer_nouvel_evenement():
    ...
    """
    data = request.json
    evenement = Evenement(id_association=data["id_association"],           nom=data["nom"], 
...
@login_required
def toggle_visibility(evenement_id):
    evenement = Evenement.query.get(evenement_id)
    if not evenement:
        return jsonify({"message": "Événement non trouvé"}), 404
    if not (current_user.est_superutilisateur or evenement.id_association in current_user.assos_actuelles.keys()):
        return jsonify({"message": "Vous n'avez pas les permissions pour modifier cet événement"}), 403
...
def supprimer_evenement(evenement_id):
    ...
    if not evenement:
        return jsonify({"message": "Événement non trouvé"}), 404
    if not (current_user.est_superutilisateur or evenement.id_association in current_user.assos_actuelles.keys()):
        return jsonify({"message": "Vous n'avez pas les permissions pour supprimer cet événement"}), 403
...
@login_required
def annuler_evenement(evenement_id):
    data = request.json
    date = data.get("date")
    evenement = Evenement.query.get(evenement_id)
    if not evenement:
        return jsonify({"message": "Événement non trouvé"}), 404
    if not (current_user.est_superutilisateur or evenement.id_association in current_user.assos_actuelles.keys()):
        return jsonify({"message": "Vous n'avez pas les permissions pour annuler cet événement"}), 403
...
@login_required
def obtenir_evenements(date:str) :
    """
